...
void drawMainScreen() {
  ...
  if (!rtcAvailable) {
    display.print("Time: NO RTC");
  } else {
    char timeStr[25];
    snprintf(timeStr, sizeof(timeStr), "Time: %02d:%02d:%02d %s", 
             getDisplay12Hour(now.hour()), now.minute(), now.second(),
             getAmPm(now.hour()));
  ...
  display.setTextSize(1);
  display.print("Prev Hour:");
  display.setCursor(0, 40);
  display.setTextSize(2);
  display.print(hourlyCount);
  ...
  if (statusMessage.length() > 21) {
    display.print(statusMessage.substring(0, 18));
    display.print("...");
  } else {
